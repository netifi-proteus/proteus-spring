...
public class BrokerClientAutoConfiguration {
    ...
    public BrokerClientConfigurer propertiesBasedBrokerClientConfigurer(BrokerClientProperties brokerClientProperties) {
        return builder -> {
            ...
            if (!StringUtils.isEmpty(broker.getHostname())) {
                ...
                switch(discovery.getEnvironment()) {
                    case "static":
                        BrokerClientProperties.DiscoveryProperties.StaticProperties
                            staticProperties = discovery.getStaticProperties();
                        StaticListDiscoveryConfig
                            staticListDiscoveryConfig = new StaticListDiscoveryConfig( staticProperties.getPort(), staticProperties.getAddresses());
                        ...
                        break;
                    case "ec2":
                        BrokerClientProperties.DiscoveryProperties.EC2Properties ec2Properties = discovery.getEc2Properties();
                        EC2TagsDiscoveryConfig
                            ec2TagsDiscoveryConfig = new EC2TagsDiscoveryConfig(ec2Properties.getTagName(), ec2Properties.getTagValue(), ec2Properties.getPort());
                        discoveryStrategy = DiscoveryStrategy.getInstance(ec2TagsDiscoveryConfig);
                        connectionType = ec2Properties.getConnectionType();
                        break;
                    case "consul":
                        BrokerClientProperties.DiscoveryProperties.ConsulProperties
                            consulProperties = discovery.getConsulProperties();
                        ConsulDiscoveryConfig
                            consulDiscoveryConfig = new ConsulDiscoveryConfig( consulProperties.getConsulURL(), consulProperties.getServiceName());
                        discoveryStrategy = DiscoveryStrategy.getInstance(consulDiscoveryConfig);
                        connectionType = consulProperties.getConnectionType();
                        break;
                    case "kubernetes":
                        BrokerClientProperties.DiscoveryProperties.KubernetesProperties
                            kubernetesProperties = discovery.getKubernetesProperties();
                        KubernetesDiscoveryConfig
                            kubernetesDiscoveryConfig = new KubernetesDiscoveryConfig(kubernetesProperties.getNamespace(), kubernetesProperties.getDeploymentName(), kubernetesProperties.getPortName());
    ...
    public static class TracingConfigurations {
        ...
        public Tracer tracer(
            ...
            return new BrokerTracerSupplier(
                brokerClient,
    ...
    static BrokerClient configureBrokerClient(
        List<? extends BrokerClientConfigurer> configurers
    ) {
