...
public class BrokerClientStaticFactory {
    ...
    private static final Logger LOGGER =
        LoggerFactory.getLogger(BrokerClientStaticFactory.class);

    /**
     * Creates an instance of the correct Netifi Broker client for injection into a
     * annotated field.
     *
     * @return an instance of a {@link com.netifi.broker.BrokerClient} client
     */
    public static Object getBeanInstance(final DefaultListableBeanFactory beanFactory,
        ...
        if (!beanFactory.containsBean(beanName)) {
            ...
            if (brokerClientAnnotation.tagSupplier() != null && !brokerClientAnnotation.tagSupplier()
                                                                                         .equals(
                                                                                             NoTagsSupplier.class)) {
            ...
            for (Tag t : brokerClientAnnotation.tags()) {
                if (!suppliedTags.stream()
                                 .anyMatch(tag -> tag.getKey()
                                                     .equals(t.name()))) {
                    suppliedTags =
                        suppliedTags.and(com.netifi.common.tags.Tag.of(t.name(),
                            t.value()));
                }
            }
            ...
            try {
                ...
                if (BrokerClientFactory.class.isAssignableFrom(targetClass)) {
                    ...
                    BrokerClientFactory baseFactory = createBaseClientFactory(clientClass,
                        brokerClient,
                    ...
                    toRegister = createBrokerClient(brokerClient,
                        brokerClientAnnotation.type(),
                        brokerClientAnnotation.group(),
                        brokerClientAnnotation.destination(),
                        suppliedTags,
                ...
                throw new RuntimeException(String.format("Error injecting bean '%s'",
                    targetClass.getSimpleName()), e);
            }

            Object newInstance = beanFactory.initializeBean(toRegister, beanName);
            beanFactory.autowireBeanProperties(newInstance,
            ...
            AutowireCandidateQualifier qualifier =
                new AutowireCandidateQualifier(Qualifier.class);
            ...
            LOGGER.debug(
                "Bean named '{}' already exists, using as current bean reference.",
                beanName);
     ...
     * @return bean name
     */
    private static String getBeanName(BrokerClient brokerClientAnnotation,
        Class<?> clazz) {
        Assert.hasText(brokerClientAnnotation.group(),
            "@BrokerClient.group() must be specified");

        String beanName = clazz.getSimpleName() + "_" + brokerClientAnnotation
            .type()
            .toString()
            .toLowerCase() + "_" + brokerClientAnnotation.group();
    ...
    private static <T> T createBrokerClient(com.netifi.broker.BrokerClient brokerClient,
        ...
        switch (routeType) {
            ...
            case GROUP:
                ...
                break;
            case DESTINATION:
                brokerSocket = brokerClient.groupServiceSocket(group,
        ...
        if (tracer == null && meterRegistry == null) {
            ...
            // MeterRegistry Only
            Constructor ctor =
                clientClass.getConstructor(RSocket.class, MeterRegistry.class);
    ...
    private static BrokerClientFactory createBaseClientFactory(Class<?> clientClass,
        com.netifi.broker.BrokerClient brokerClient,
        BrokerClient.Type routeType,
        String group,
        String destination,
        ...
        BrokerClientFactory baseFactory = new BrokerClientFactory() {
            ...
            public Object lookup(BrokerClient.Type type,
                ...
                if (instantiatedClients.containsKey(key)) {
                    ...
                    try {
                        client = createBrokerClient(brokerClient,
                            routeType,
                            methodGroup,
                            destination,
                            methodTags,
